...
if __name__ == '__main__':
    ...
    with open("output1.txt", "r") as f1:
        ...
        for line in lines:
            ...
            # if sum == 1:
            #     print(connection)
            list_nodes_values0.append([int(connection[0])-1, float(connection[1])])
    ...
    list_tot = sorted(list_tot, key=lambda x:x[0],reverse=True)
    ...
    y = ecdf(x)
    plt.ylim(0, 1)
    plt.plot(x, y, linewidth='1', marker='o', label='betweenness with effective_size_top20% cdf')
    ...
    y = ecdf2(x)
    plt.ylim(0, 1)
    plt.plot(x, y, linewidth='1', marker='o', label='betweenness-non-SHS cdf')

    plt.xlabel('betweenness')
    plt.ylabel('CDF')
    plt.title('betweenness CDF')
